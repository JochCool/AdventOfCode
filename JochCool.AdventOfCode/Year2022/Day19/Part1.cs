...
static class Part1
{
	public static string? Solve(TextReader inputReader)
	{
		...
		return FindMostGeodes(resourceTypes2).ToInvariantString();
		...
		foreach (string line in inputReader.ReadLines())
		{
			...
			int blueprintNum = StringUtil.ParseInvariantAt<int>(line, ref i, ':');
			...
			resourceTypes[0].OreCost = StringUtil.ParseInvariantAt<int>(line, ref i, ' ');

			i += " ore. Each clay robot costs ".Length;
			resourceTypes[1].OreCost = StringUtil.ParseInvariantAt<int>(line, ref i, ' ');

			i += " ore. Each obsidian robot costs ".Length;
			resourceTypes[2].OreCost = StringUtil.ParseInvariantAt<int>(line, ref i, ' ');
			i += " ore and ".Length;
			resourceTypes[2].SecondaryCost = StringUtil.ParseInvariantAt<int>(line, ref i, ' ');

			i += " clay. Each geode robot costs ".Length;
			resourceTypes[3].OreCost = StringUtil.ParseInvariantAt<int>(line, ref i, ' ');
			i += " ore and ".Length;
			resourceTypes[3].SecondaryCost = StringUtil.ParseInvariantAt<int>(line, ref i, ' ');
